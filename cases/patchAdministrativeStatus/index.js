...
export default async ({ body, MODELS, MODULES }) => {
    ...
    const previousValue = theCase.administrativeStatus;

    await theCaseObj.update({
        administrativeStatus: body.administrativeStatus });
    ...
    const newHistory =
        await MODULES.history.postUpdateCaseAdministrativeStatus({

        patientId: theCase.patientId,
        caseId: theCase.id,
        originOrganizationId: theCase.organizationId,
        previousValue,
        newValue: theCase.administrativeStatus,
    });
