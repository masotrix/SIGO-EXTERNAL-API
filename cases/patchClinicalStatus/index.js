...
export default async ({ body, MODELS, MODULES }) => {
    ...
    const previousValue = theCase.clinicalStatus;

    await theCaseObj.update({ clinicalStatus: body.clinicalStatus });
    ...
    const newHistory =
        await MODULES.history.postUpdateCaseClinicalStatus({

        patientId: theCase.patientId,
        caseId: theCase.id,
        originOrganizationId: theCase.organizationId,
        previousValue,
        newValue: theCase.clinicalStatus,
    });
